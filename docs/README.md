...
- [CORAL split/manifest tooling](../experiments/coral/scripts/coral_current_progression_manifest.py),
  ...
  changed inputs or policies fail. No real CORAL manifest has been generated by this checkpoint.
...
- The [current-progression implementation plan](superpowers/plans/2026-09-15-m1-current-progression-task-protocol.md)
  continues with the operator workflow and restricted-data integration/freeze checks. The protocol,
  answer value schema, parser diagnostics, candidate rules, and split tooling are implemented,
  but the clinical gates remain unevaluated and its independent human review has not been performed
